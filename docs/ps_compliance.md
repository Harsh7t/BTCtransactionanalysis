...
> figures elsewhere come from **demo** and the two do not agree — e.g. 150 behavioural
> archetypes (demo) vs 544 (bulk), and attribution top-1 0.9052 (demo, in
> `attribution_method.md`) vs 0.9243 (bulk, in `artifacts/bulk/metrics.json`). Neither is
...
> decimals and sorting by value inside a band, which is what shipped previously, cost 30
> points of precision@10 and 12 of precision@25 on the bulk test fold.
...
| 2 | "ingests bulk Bitcoin transaction/network metadata" | `ingest/parsers.py` | `make run` → 2,467,299 rows in ~2.6 min (bulk, last measured run 155.5 s); 564,303 in ~15 s (demo) | ✅ |
| 3 | "in CSV/JSON/XML" | `ingest/parsers.py:READERS` | all three parse to **identical** clean-row counts | ✅ |
| 4 | "correlates network-layer (IP/port/timing) with blockchain-layer (wallet/TXID/amount)" | `attribute/engine.py` | `metrics.json → attribution.top1_accuracy` = **0.9243** vs 0.3378 chance (bulk) | ✅ |
| 5 | "applies AI/ML to detect anomalies" | `detect/supervised.py`, `detect/novelty.py` | test PR-AUC **0.2998** vs 0.0039 base rate on bulk (77×) | ✅ |
| 6 | "cluster entities" | `graph/resolve.py` + `detect/novelty.py` | **two levels**: 829,821 address clusters on bulk (purity 0.9956) and 544 HDBSCAN behavioural archetypes | ✅ |
| 7 | "generate prioritized, explainable investigative leads" | `pipeline.py`, `explain/` | ranked queue, isotonic-calibrated (ECE 0.0381 on bulk), exact SHAP → narrative | ✅ |
...
| 10 | "Implement AI/ML detection use case with a **working model — not just rules**" | `detect/` | ablation on bulk: rules alone **0.0030** (below the 0.0039 base rate), model **0.2998** | ✅ |
| 11 | "ranked, explainable alert list (why a **wallet/transaction** was flagged, with a confidence score)" | `pipeline.py`, `detect/transaction_head.py` | `/api/alerts` (entity) and `/api/transactions` (TXID, PR-AUC 0.0726 vs 0.0015 base on bulk; 0.1041 vs 0.0121 on demo) | ✅ |
| 12 | "Present findings via a simple dashboard or link-analysis visualization" | `ui/` | Alerts (table · dossier · focus layouts), case file, link-analysis graph, Model and Provenance screens; the case file carries evidence with real TXIDs | ✅ |
...
| # | PS wording | Implemented in | Verified by | Status |
|---|---|---|---|---|
| 13 | "synthetic dataset modelled on real Bitcoin P2P/transaction fields" | `generator/` (1,701 lines) | `make generate`; parameters documented in `docs/generator_parameters.md` | ✅ |
...
| # | PS wording | Implemented in | Verified by | Status |
|---|---|---|---|---|
| 16 | "Workable complete offline solution for **linux platform**" | `Dockerfile`, `wheels/` | **executed**: `make docker-build && make docker-verify` → 5/5 steps pass under `--network none` on linux/amd64 — run when the suite had 81 tests (88 now; the container run has not been repeated since), LightGBM 3.3.5 resolved, GeoIP offline, full pipeline on 564k rows, and container scores bit-identical to the host | ✅ |
| 17 | "Working prototype (**code repo**) with ingestion, correlation, and AI/ML model" | git repo, 88 tests | `make test` → 88 passed | ✅ |
